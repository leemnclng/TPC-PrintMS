...
def test_uploaded_docx_is_auto_converted_to_pdf_before_printing(tmp_path, monkeypatch) -> None:
    engine = create_engine(
        f"sqlite:///{tmp_path / 'docx-conversion.db'}",
        connect_args={"check_same_thread": False},
    )
    test_session = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    Base.metadata.create_all(engine)
    ...
    paper = _create_material(client, headers, "Letter DOCX paper", "sheet", 100, paper_size="Letter")
    _assign_pricing_materials(client, headers, "printing", [paper["id"]])
    rules = client.get("/document-analyzer/pricing-rules", headers=headers).json()
    bw_rule = next(rule for rule in rules if rule["paperSize"] == "Letter" and rule["printType"] == "black_and_white" and rule["pricingScope"] == "printing")
    assert client.put(
    ...
    service = client.post(
        "/services",
        headers=headers,
        json={"name": "DOCX printing", "category": "printing", "isActive": True},
    ).json()
    product = client.post(
        "/products",
        headers=headers,
    ...
    docx_document.add_heading("Customer letter", level=1)
    docx_document.add_paragraph("Attached as a Word document; must print like any other job.")
    docx_buffer = BytesIO()
    ...
    transaction_response = client.post(
        ...
        files=[("files", ("customer-letter.docx", docx_bytes, "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))],
    )
    assert transaction_response.status_code == 201, transaction_response.text
    order = transaction_response.json()

    # The retained print-ready file is a converted PDF, not the original
    # DOCX, so printing (which only accepts PDF/image files) works unmodified.
    assert order["files"][0]["kind"] == "print_ready"
    assert order["files"][0]["originalFilename"] == "customer-letter.pdf"
    retained = client.get(
        f"/job-orders/{order['id']}/files/{order['files'][0]['id']}",
        headers=headers,
    )
