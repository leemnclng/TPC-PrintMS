...
function scannerHasDetectedOriginal(device?: ScannerDeviceState): boolean {
  return Boolean(device?.supportsFeeder && device.detectsFeeder && device.feederReady);
}

function scannerRequiresPlacementConfirmation(device?: ScannerDeviceState): boolean {
  if (!device || scannerHasDetectedOriginal(device)) return false;
...
function scannerReportsNoOriginal(device?: ScannerDeviceState): boolean {
  return Boolean(device?.supportsFeeder && !device.supportsFlatbed && device.detectsFeeder && !device.feederReady);
}
...
export function PhotocopyJobCreateModal({ open, service, customers, products, inventoryItems, pricingRules, onClose, onCreated }: Props) {
  ...
  const requiresPlacementConfirmation = scannerRequiresPlacementConfirmation(selectedScanner);
  const reportsNoOriginal = scannerReportsNoOriginal(selectedScanner);
  const placementReady = scannerHasDetectedOriginal(selectedScanner) || (requiresPlacementConfirmation && placementConfirmed);
  const hardwareReady = Boolean(selectedScanner?.isOnline && !selectedScanner.coverOpen && !selectedScanner.paperJam && !reportsNoOriginal && (selectedScanner.supportsFlatbed || selectedScanner.supportsFeeder));
  ...
  async function acquirePage() {
    ...
    setError(null);
    try {
      const requestedSettings = { source: "auto" as const, ...scanSettings, placementConfirmed: scannerHasDetectedOriginal(selectedScanner) || placementConfirmed };
                  ...
                  <>
                    <section className={`scan-acquisition${submitted && !scanCaptures.length ? " is-invalid" : ""}`} aria-invalid={submitted && !scanCaptures.length} tabIndex={submitted && !scanCaptures.length ? -1 : undefined}>
                      ...
                      <div className="scan-preflight">
                        ...
                        {requiresPlacementConfirmation ? <label className={`scan-placement-confirm${placementConfirmed ? " is-confirmed" : ""}`}><input type="checkbox" checked={placementConfirmed} onChange={(event) => { setPlacementConfirmed(event.target.checked); setScannerError(null); }} disabled={acquiring || saving} /><span><strong>Document is loaded</strong><small>Confirm the original is on the glass or in the feeder. This scanner cannot report placement for every source.</small></span></label> : null}
...
function ScannerReadiness({ device }: { device: ScannerDeviceState }) {
  ...
  } else if (scannerReportsNoOriginal(device)) {
    tone = "is-error";
    title = "No document detected";
    detail = "Load an original in the feeder, then refresh readiness.";
  } else if (scannerRequiresPlacementConfirmation(device)) {
    tone = "is-manual";
    title = "Placement confirmation required";
    detail = `${device.name} cannot verify an original on every source. Place it on the glass or in the feeder, then confirm below.`;
  } else {
    tone = "is-error";
