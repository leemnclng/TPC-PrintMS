...
export function JobScanSetupModal({ open, order, onClose, onScanned }: Props) {
  ...
  async function acquirePage() {
    ...
    try {
      ...
      await addScanFiles([new File([bytes], result.file.filename, { type: result.file.mimeType })], "scanner", appliedSettings);
      setPlacementConfirmed(false);
      await refreshScanners(false);
      setScanFeedback({ tone: "success", title: "Scan completed", detail: `${result.file.filename} was acquired from the ${scanSourceLabel(appliedSettings.source)}. ${result.message ?? "Review the preview, then scan another page or submit the softcopy."}` });
    } catch (caught) {
