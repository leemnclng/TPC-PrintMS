  ...
  function lineContext(line: TransactionLine) {
    ...
    if (product?.operationKind === "printing") {
      ...
      const customRate = product.documentRates.find((candidate) =>
        pricingRules.some((rule) => rule.id === candidate.pricingRuleId && rule.inventoryItemId === line.paperId),
      )?.pricePerPage;
      const globalRate = pricingRules.find((rule) =>
        rule.isActive && rule.inventoryItemId === line.paperId && rule.printType === product.printType,
      )?.pricePerPage;
