  ...
  return (
    <>
      ...
      <div className="job-command-grid">
        ...
        <Card>
          <CardHeader title="Production brief" meta={`${usedMaterials}/${plannedMaterials.length} materials used`} />
      ...
      <details className="job-audit-panel">
        ...
        <div className="job-audit-grid">
          ...
          <section>
            ...
            {materialMovements.length ? materialMovements.map((movement) => <div className="job-audit-row" key={movement.id}><span><strong>{movement.inventoryItemName}</strong><small>{formatDateTime(movement.occurredAt)} · balance {movement.balanceAfter}</small></span><b>{Math.abs(movement.quantityDelta)} {movement.inventoryItemUnit}</b></div>) : <p>Materials deduct automatically after printer acceptance.</p>}
          </section>
          <section>
