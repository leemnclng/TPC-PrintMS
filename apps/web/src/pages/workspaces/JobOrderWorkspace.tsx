...
export function JobOrderWorkspace() {
  ...
  return (
    <>
      ...
      {initialService ? <TransactionCreateModal open={addProductsOpen} order={order} initialService={initialService} services={services} products={products} inventoryItems={inventoryItems} pricingRules={pricingRules} scanPricingTiers={scanPricingTiers} sourceSpoolerJobId={attachedSpoolerJobId} sourceObservedPrintJob={(spoolerMonitor?.jobs ?? []).find((job) => job.id === attachedSpoolerJobId) ?? null} otherObservedPrintJobs={otherObservedPrintJobs} customers={[]} onClose={() => { setAddProductsOpen(false); setAttachedSpoolerJobId(null); }} onCreated={() => { setAddProductsOpen(false); setAttachedSpoolerJobId(null); reload(); }} /> : null}
