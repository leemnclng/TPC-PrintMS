...
export function ExternalPrintPrompt() {
  ...
  async function deferNotification() {
    if (!job || deferring) return;
    setDeferring(true);
    setError(null);
    try {
      await api.post<SpoolerMonitorInfo>(`/printers/spooler-jobs/${encodeURIComponent(job.id)}/defer-notification`);
      suppressed.current.add(job.id);
      setJob(null);
    } catch (caught) {
      setError(caught instanceof ApiError ? caught.message : "The notification could not be hidden.");
    } finally {
      setDeferring(false);
    }
  }
    ...
    <aside className="external-print-prompt" aria-labelledby="external-print-prompt-title" aria-live="polite">
      ...
      <div className="external-print-prompt__body">
        ...
        <div className="external-print-prompt__actions">
          ...
          <Button type="button" variant="secondary" size="sm" onClick={attachToExistingOrder} disabled={deferring}>Add to order</Button>
          <Button type="button" variant="primary" size="sm" onClick={startJobOrder} disabled={deferring}>Create job</Button>
        </div>
      </div>
    </aside>
  );
