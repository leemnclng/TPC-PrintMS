...
Use this checklist when preparing a Windows workstation for OMS. The
...
2. Install the current Canon driver package for the exact printer model. On the
   ...
   scanner available to OMS through Windows Image Acquisition (WIA).
...
instead of running them. The script creates or replaces `OMS.lnk` and removes
the former `Printing-MS.lnk` from the current user's Windows Desktop, launching `scripts/run.sh` through Git Bash (found
automatically), and works with repository paths containing spaces. Closing
the final OMS window stops its managed backend and also closes the
shortcut-opened Git Bash terminal.

Always obtain the Canon package from Canon's support page for the exact model
and Windows version. Do not redistribute a vendor driver inside the OMS
...
4. Allow Canon utilities and OMS through Windows Firewall on the local
...
- OMS Scan intake lists the scanner after **Refresh devices**.
...
Canon's own discovery stack, while OMS enumerates WIA devices. If Canon
PRINT works but Windows Scan and OMS do not, install or repair the
...
| --- | --- | --- |
| Canon PRINT scans, but OMS reports no scanner | Canon software can reach the device, but no WIA scanner is registered | Install/repair IJPAT or the full Canon MP/WIA package, restart Windows, then test with Windows Scan |
...
| Scanner is visible but acquisition fails | Device is busy or reports paper, cover, or feeder state | Follow the OMS readiness message and retry |
...
| Window turns black, then white | The Electron GPU or renderer process exited | Update/rebuild OMS; Windows software rendering is now the default. Check Electron's `desktop.log` and the active stage's `.data\<stage>\logs\backend.log`. Test hardware acceleration only with `PRINTING_MS_ENABLE_HARDWARE_ACCELERATION=1`. |
