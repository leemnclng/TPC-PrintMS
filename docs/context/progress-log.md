...
## 2026-09-13 (Ad Hoc materials without a paper size are now priceable)

- Summary: An owner selected 7 materials for a custom "Lamination" (Ad Hoc) pricing category but only 4 — the ones tagged with a paper size — ever got a priceable rate row; the other 3 (pouches/film) couldn't be priced at all. Root cause was `PricingService.ensure_defaults()` (and matching gates in `job_orders.py`/`products.py`) requiring a real paper size before a material could be priced, inherited unchanged from Printing/Photocopy even though Ad Hoc has no physical paper to feed.
