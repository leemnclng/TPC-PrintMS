...
- Decision: Recording a purchase remains financial-only, but each purchase may be explicitly applied once from its linked material to create usable stock. A ream purchase snapshots its own sheets-per-ream value instead of inheriting future material changes.
- Rationale: Purchased package sizes vary by delivery, and expenditure entry should not silently alter transaction stock. Explicit application connects the physical stock received to one auditable purchase.
- Impact: Every purchase exposes its UUID and availability. Applying it adds `quantity_purchased × sheets_per_ream` for reams, or the purchased quantity for matching unit purchases, and creates a stock-in movement linked to the UUID. Applied purchases are retained and cannot be deleted or applied again; unapplied incorrect entries remain deletable. The material's current purchase-price reference is unchanged. Weighted-average valuation and profit reporting remain future work.
...
## 2026-09-13 — Auto-convert DOCX (not XLSX/PPTX) to PDF before printing, using a pip-only library

- Decision: When a DOCX is uploaded for a Printing product, convert it to PDF with `dxpdf` (a pip-installable, MIT-licensed, Rust/Skia-based converter with no external app dependency) immediately after document analysis, and store the converted PDF — not the original DOCX — as the job's print-ready file. XLSX and PPTX are not auto-converted.
- Rationale: The Windows print path already refuses to print anything but PDF/image files, so a stored DOCX was unprintable without the owner manually exporting it first. LibreOffice headless conversion was evaluated first and works for all three Office formats, but was rejected because it requires installing a large separate application on the print-shop workstation. Microsoft Office COM automation (`docx2pdf` + `pywin32` for Excel/PowerPoint) was rejected because it requires MS Office to already be installed, which this deployment does not have. The free tiers of commercial SDKs (Spire.Doc/Cells/Presentation) were rejected because they watermark or truncate real documents, making them unusable for actual customer print jobs. No usable pip-only converter exists for XLSX or PPTX today.
- Impact: `services/api/app/modules/document_analyzer/utils/office_conversion.py` holds the conversion and the `CONVERTIBLE_TO_PDF_SUFFIXES` set (currently `{".docx"}`); `job_orders.py` calls it right after `analysis_service.analyze` succeeds in both the analyzed-job and transaction upload flows, before the file is written to disk. A failed conversion returns a 422 instead of silently storing an unprintable file. XLSX/PPTX uploads continue to analyze and price normally, but printing still asks the owner to export those to PDF first — revisit if a real free converter appears, or if the shop later has LibreOffice/MS Office installed anyway.

## 2026-09-13 — Ad Hoc materials are priceable without a paper size

- Decision: `PricingService.ensure_defaults()` now creates a `DocumentPricingRule` for every material assigned to an **Ad Hoc** pricing category, not only ones tagged with a paper size. Printing and Photocopy are unchanged — both still require a real paper-tagged material, since Printing's analyzer fits pages against physical dimensions and Photocopy's device flow always feeds real paper. `DocumentPricingRule.paper_size` (and the API/frontend types built on it) is now nullable to represent this.
