...
| Job Orders | One-page transaction workflow with human-readable names, scalable numbering, modal creation, payment, concurrent queueing, supervised printing, quality review, completion, and compact audit history | Active | Creation suggests a required editable name from the uploaded filename. The name is the primary operational label; the transactional 10-digit reference remains visible as secondary audit metadata and internal identities remain UUIDs. Every queued/printing order appears by name in global activity, with attention links returning to its workflow. |
