...
| Reports | Interval-based sales, quality, reconciliation, printer reliability, and inventory reports | Active | Owners select explicit From/To dates. Reports reconcile successful OMS attempts against Windows spooler releases, flag unconfirmed/mismatched output and unreviewed external leakage, aggregate structured failures and material waste, break reliability down by printer, and export each new section as CSV. Non-Windows hosts identify spooler data as unavailable. Existing sales, re-attempt, discount, and live inventory reports remain active. |
