...
| Printer Integration | Vendor-neutral discovery, concurrent submission, supervised manual duplex, photo-media profiles, and Windows spooler observation | Active | Print Center highlights the OS default printer and separates selectable alternatives under Others. Photo Print attempts retain Canon-style named media, high-quality/crop/borderless defaults, and send standard media hints to the OS driver. Multiple job orders may be submitted while the OS printer queue processes earlier work. Internal Windows attempts retain queued/spooling/printing/released state and page progress. Release requests owner review rather than claiming physical completion. Supervised duplex retains its front/reinsert/back checkpoint and deducts inventory only after both submissions. |
...
| Settings | Business profile, environment storage, verified backup/restore, stage-separated database paths, and backend diagnostics | Active | Each development, test, or production folder contains its SQLite database, managed files, backups, and non-secret `config.json`. Settings creates checksum-verified ZIP backups, retains a local copy, and restores only matching-stage archives after creating a safety backup. `PRINT_MS_STAGE` still selects the active boundary on restart; live switching remains disabled. Document templates are planned. |
