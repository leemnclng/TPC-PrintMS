...
| Printer Integration | Vendor-neutral discovery and confirmed file submission through Windows and macOS/Linux queues | Active | Print Center detects the host OS, selects an available queue, file, copies, color mode, and paper size, and submits only a paid/queued job after explicit owner confirmation. Windows renders PDF/image pages locally at print resolution and sends them through `PrintDocument` to the selected installed driver, avoiding file-association `PrintTo` failures; CUPS uses `lp`. Successful and failed attempts retain printer, file, settings, operator, result, OS job ID when available, and concise error detail. Physical revalidation of the new Canon path remains open. |
