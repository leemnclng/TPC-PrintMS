...
| Job Orders | Service-first modal intake with Printing, Photocopy, and Scan requirements, human-readable names, payment, completion, deliverables, and audit history | Active | On Windows, Scan jobs configure content mode, 150/300/600 DPI, and Automatic/A4/Letter/Legal/photo capture sizes in-app, then acquire directly through WIA without opening its redundant settings dialog. Source readiness, jam/cover/offline errors, PNG preview normalization, merging, pricing, and retained softcopy remain integrated. The Canon workstation requires its model-specific IJPAT/full MP/WIA package. |
