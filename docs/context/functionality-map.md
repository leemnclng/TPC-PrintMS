...
| Pricing | App-managed global additions, product discounts, product-aware rate matrices, searchable category material assignments, and a centralized read-only price book | Active | Pricing categories may assign any active inventory material. Paper assignments create per-page rate rows, while ink, toner, binding, laminate, and other assignments define the additional supplies available to new products in that category. Material assignment includes search by name, category, unit, or paper size. Printing and Scan or Photocopy remain seeded categories, and owners may create additional compatible categories. |
