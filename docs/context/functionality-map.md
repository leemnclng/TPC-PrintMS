...
| Job Orders | Multi-service transaction intake with independent product operations, combined payment/completion, audited void-and-correction, product repricing/cancellation, quality reprocessing, deliverables, and audit history | Active | The register and tracked-print attachment picker use server-side search/filter/sort, 25/50/100-row paging, and next-page prefetch. Complete reference collections remain available to transaction forms. Paid/completed transactions retain the existing guarded void, correction, payment, production, and audit workflows. |
