...
| Job Orders | Service-first modal intake with Printing, Photocopy, and Scan requirements, human-readable names, payment, completion, deliverables, and audit history | Active | On Windows, Scan jobs discover WIA devices, automatically select a driver-reported loaded feeder/flatbed or defer to the vendor dialog, and map jam/cover/offline errors into recovery guidance without a manual placement gate. WIA output is normalized to PNG for reliable preview before pages are merged, priced, and retained without inventory usage. The Canon workstation requires its model-specific IJPAT/full MP/WIA package. |
