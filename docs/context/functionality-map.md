...
| Printer Integration | Vendor-neutral discovery and confirmed file submission through Windows and macOS/Linux queues | Active | Print Center highlights the OS default printer and separates selectable alternatives under Others. Paper, copies, and output mode remain authoritative from the approved transaction; the owner can set orientation, scaling, quality, borderless behavior, and collation per attempt. Windows applies these through `PrintDocument` and can open the selected driver's preferences for Canon-specific media/tray settings; CUPS uses equivalent `lp` options. Queue acceptance advances the job and atomically records planned inventory usage. |
...
| Configuration | App-managed print-type catalog, global variants, global document-analyzer per-page pricing, and per-product rate overrides | Active | The pricing matrix gets one column per active print type and one row per stocked paper size. “Add print type” defines the label, automatic printer color mode, and optional measured-ink pricing; its paper rates and product option then appear automatically. Product overrides remain limited to the product's selected type. |
