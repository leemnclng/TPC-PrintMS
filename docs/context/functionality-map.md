...
| Configuration | App-managed print-type catalog, global variants, global document-analyzer per-page pricing, per-product rate overrides, and a centralized read-only Pricing Center | Active | Pricing Center mirrors every service from Printing Services and groups its products into a rate table covering paper sizes, scan rates, print types, variants, missing/custom values, and direct configuration links. The editable configuration matrix retains one column per active print type and one row per stocked paper size. |
...
| Job Orders | Multi-service transaction intake with independent product operations, combined payment/completion, deliverables, and audit history | Active | The owner chooses an initial service, then may add products from any active service. Printing lines require their own uploaded/analyzed document; Scan lines acquire inside the saved transaction; Photocopy lines record device-side completion. Each line owns Queued/Printing/Ready progress, files, printer attempts, and material deduction. The transaction owns the combined price, payment, and final completion. |
