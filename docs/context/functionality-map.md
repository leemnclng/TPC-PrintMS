...
| Job Orders | Service-first modal intake with Printing, Photocopy, and Scan requirements, human-readable names, payment, completion, deliverables, and audit history | Active | On Windows, Scan jobs discover WIA devices, validate connection/source readiness, require placement confirmation where hardware sensing is unavailable, and map feeder/jam/cover/offline errors into recovery guidance. Acquired pages are previewed, merged, priced automatically, and retained without inventory usage. |
