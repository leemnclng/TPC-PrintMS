...
| Service Catalog | Full service create/read/update/delete with Printing, Photocopy, and Custom workflow categories, nested product CRUD, app-configured print types, variants, and paper pricing | Active | Existing services migrate as Printing. Photocopy B&W products require independent per-paper rates; color photocopy products may use global rates. Custom services are catalog-ready but have no built-in job workflow yet. |
...
| Job Orders | Service-first modal intake with workflow-specific requirements, human-readable names, scalable numbering, payment, production, completion, and audit history | Active | Printing requires upload and analysis. Photocopy requires product, paper, pages, copies, and optional back-to-back input; it creates no file, computes price live, deducts paper, and opens Ready for payment. Custom workflows remain intentionally unavailable. |
