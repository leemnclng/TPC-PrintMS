...
| Job Orders | Service-first modal intake with Printing, Photocopy, and Scan requirements, human-readable names, payment, completion, deliverables, and audit history | Active | Scan jobs attach the PDF/image generated by the scanner, verify its pages, retain it as a downloadable softcopy, charge the product's standalone per-page rate, consume no inventory, and open Ready for payment. |
