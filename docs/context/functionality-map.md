...
| Reports | Daily, weekly, and monthly sales, quality re-attempt, and live inventory reports | Active | Period boundaries follow the workstation's local calendar. Sales total verified payments recorded in-period; re-attempts count product lines moved from Ready back to Queued by failed quality. Inventory is a current active-material snapshot classified as Healthy, Low, or Out of stock. Previous/next period navigation, explicit generation, loading, retry, and honest empty states are active. |
